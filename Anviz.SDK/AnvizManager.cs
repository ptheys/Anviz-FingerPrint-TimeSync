...
namespace Anviz.SDK
{
    public class AnvizManager
    {
        ...
        private const byte GET_DATE_TIME = 0x38;
        private const byte SET_DATE_TIME = 0x39;
        ...
        {
            byte[] response = SendCommand(GET_DATE_TIME, deviceId, new byte[] { }, 0);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
                var year = (byte)ReadBytes(SplitBytes(parsed.DATA, 0, 1));
                var month = (byte)ReadBytes(SplitBytes(parsed.DATA, 1, 1));
                var day = (byte)ReadBytes(SplitBytes(parsed.DATA, 2, 1));
                var hour = (byte)ReadBytes(SplitBytes(parsed.DATA, 3, 1));
                var minute = (byte)ReadBytes(SplitBytes(parsed.DATA, 4, 1));
                var second = (byte)ReadBytes(SplitBytes(parsed.DATA, 5, 1));
        ...
        public bool SetDateTime(DateTime currentDateTime)
        {
            var data = new[]
            {
                (byte)(currentDateTime.Year - 2000), (byte)currentDateTime.Month, (byte)currentDateTime.Day, (byte)currentDateTime.Hour,
                (byte)currentDateTime.Minute, (byte)currentDateTime.Second
            };

            var response = SendCommand(SET_DATE_TIME, deviceId, data, 6);
            if (response != null)
            {
                Response parsed = GenerateResponse(response);
